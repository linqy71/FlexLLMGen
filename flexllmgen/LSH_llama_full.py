"""
Usage:
python3 -m flexllmgen.LSH_llama_full --model meta/llama3.1-8b --input dapr \
    --gpu-batch-size 1 --percent 100 0 100 0 100 0 \
    --path=<HF_HOME>/hub --prompt-len 4000 --gen-len 32 \
    --collision-threshold 3 --offload-dir <offload_dir>
"""
...
from flexllmgen.llama_config import LlamaConfig, preset_llama3_config, get_llama_config

from flexllmgen.metadata_manage import RadixTree
...
def set_cpu_affinity(gpu_id, cpu_cores=None):
    ...
    if cpu_cores is None:
        if gpu_id < 4:
            ...
            cpu_cores = list(range(32, 64)) + list(range(96, 128))
...
class SelfAttention:
    ...
    def init_weight(self, weight_home, path):
        ...
        kv_dim = h // self.config.n_head * self.config.num_key_value_heads
        weight_specs = [
            # i_n
    ...
    def alloc_prefix_kv(self, matched_prefix):
        ...
        ## note that shape uses num_key_value_heads
        shape = (total_common_len, batch_size * num_key_value_heads, head_dim)
...
class MLP:
    ...
    def init_weight(self, weight_home, path):
        h, dtype = (self.config.hidden_size, self.config.dtype)
        path = os.path.join(os.path.join(path, f"decoder.layers.{self.layer_id}."))
        if "llama3.1-8b" in self.config.name:
            weight_specs = [
                # pos_n
        ...
        elif "llama3.3-70b" in self.config.name:
            weight_specs = [
                # pos_n
                ((h, ), dtype, path + "post_attn_layernorm.weight"),
                # gate
                ((28672, h), dtype, path + "mlp.gate_proj.weight"),
                # up
                ((28672, h), dtype, path + "mlp.up_proj.weight"),
                # down
                ((h, 28672), dtype, path + "mlp.down_proj.weight"),
            ]
        else:
            raise ValueError(f"model type not supported:{self.config.name}")
        weights = init_weight_list(weight_specs, self.policy, self.env)
        weight_home.store(weights)
...
class LLAMA:
    def __init__(self,
        ...
        print("config layers:", self.config.num_hidden_layers, "actual layers:", self.num_layers)
        ...
        self.kv_server = LSHServer(self.config, self.num_hidden_layers, self.kv_store_path,
            K=K, L=L, batch_size=1, max_length=8192, device='cuda:0',
            collision_threshold=collision_threshold, merge=merge)
        self.set_kv_server()
    ...
    def finish_one_query(self):
        ...
        if self.kv_server.prefix_lens and self.kv_server.imp_lens:
            ...
            print(f"avg common len: {avg_prefix}; avg imp len: {avg_imp}; retention: {avg_imp/avg_prefix:.4f}")
...
def process_full_dapr():
    """Process all documents with >= 5 queries from DAPR ConditionalQA."""
    RootPath = HF_ROOT + "datasets/"
    docs = load_dataset(RootPath + "UKPLab___dapr/ConditionalQA-docs/0.0.0/67ae3daa13596700976d20605630f5f9db3bd732/", split="test")
    qrels = load_dataset(RootPath + "UKPLab___dapr/ConditionalQA-qrels/0.0.0/67ae3daa13596700976d20605630f5f9db3bd732/", split="test")
    queries = load_dataset(RootPath + "UKPLab___dapr/ConditionalQA-queries/0.0.0/67ae3daa13596700976d20605630f5f9db3bd732/", split="test")
...
def process_full_longbench():
    """Process NarrativeQA from LongBench, grouping by context with >= 10 questions."""
    RootPath = HF_ROOT + "datasets/THUDM___long_bench/data/"
    task = "narrativeqa"
...
def remove_pt_files(folder_path):
    total_size = 0
    for file in os.listdir(folder_path):
        if file.endswith('.bin'):
            ...
            os.remove(file_path)
    print(f"Total size removed: {total_size / 1000 / 1000 / 1000:.3f} GB")


def reset_all_timers():
    """Reset all fine-grained timers between queries."""
    for name in ["generate", "imp io", "imp calc", "lsh calc", "compute",
                  "imp load and compute", "copy prefix", "cache store",
                  "io part test", "load LSH meta", "load table", "build table",
                  "avgk", "hash compute", "id retrieve"]:
        timers(name).reset()


def print_all_timers():
    """Print all fine-grained timer results."""
...
def run_full_dapr_flexllmgen(args):
    """Run full DAPR evaluation: multiple documents, each with multiple queries."""
    ...
    model = LLAMA(llama_config, env, args.path, args.offload_dir, policy,
                  ...
                  merge=args.merge, K=args.K, L=args.L,
                  collision_threshold=args.collision_threshold)

    requests = process_full_dapr()
    print(f"Total documents: {len(requests)}", flush=True)
    ...
    for doc_id, (context, questions) in requests.items():
        ...
        print(f"Doc {doc_id}: prefix len = {len(prefix_input[0])}")
        ...
        ### remove persisted files and reset for next document
        remove_pt_files(os.path.join(args.offload_dir, "kv_store_llama"))
...
def run_full_longbench_flexllmgen(args):
    """Run full LongBench evaluation: multiple contexts, each with multiple questions."""
    ...
    print(f"Total contexts: {len(requests)}", flush=True)

    for doc_id, (context, questions) in requests.items():
        prefix_input = get_tokenized_inputs(context, max_prompt_len=max_prompt_len, tokenizer=tokenizer)
        print(f"Context {doc_id}: prefix len = {len(prefix_input[0])}")
        output_ids = model.generate(
            prefix_input, max_new_tokens=1, debug_mode=args.debug_mode,
            cut_gen_len=cut_gen_len, verbose=args.verbose
        )
        ...
        ### remove persisted files and reset for next context
        remove_pt_files(os.path.join(args.offload_dir, "kv_store_llama"))
        del model.radix_tree
        model.radix_tree = RadixTree()
        model.kv_server.reset(switch=True)
...
def run_dapr_flexllmgen(args):
    """Run single-document DAPR evaluation (original behavior)."""
...
def add_parser_arguments(parser):
    parser.add_argument("--model", type=str, default="meta/llama3.1-8b",
        help="The model name.")
    ...
    parser.add_argument("--tokenizer-path", type=str,
        default=HF_ROOT + "hub/models--meta-llama--Llama-3.3-70B-Instruct/snapshots/6f6073b423013f6a7d4d9f39144961bfbfbc386b",
        help="Path to the tokenizer (HuggingFace format).")
    ...
    parser.add_argument("--input", type=str, default="full_dapr",
        choices=["dapr", "full_dapr", "full_longbench"],
        help="Evaluation input type.")
